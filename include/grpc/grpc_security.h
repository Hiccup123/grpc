...
/** Set grpc_tls_key_materials_config instance with provided a TLS certificate.
   ...
   caller and the caller is free to deallocate it immediately after the call. It
   returns 1 on success and 0 on failure. It is used for experimental purpose
   for now and subject to change.
 */
...
/** A struct containing all information necessary to schedule/cancel a
    ...
    - key_materials_config is an in/output parameter containing currently
      used/newly reloaded credentials. If credential reload does not result in a
   new credential, key_materials_config should not be modified. The same
   key_materials_config object can be updated if new key materials is available.
